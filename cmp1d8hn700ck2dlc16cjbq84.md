---
title: "OIC Pattern Mastery Series - Part 1: The Multi-Device Broker"
datePublished: 2026-03-31T05:00:00.000Z
cuid: cmp1d8hn700ck2dlc16cjbq84
slug: oic-pattern-mastery-series-part-1-the-multi-device-broker
cover: https://cdn.hashnode.com/uploads/covers/69ce72fa0ff860b6ded94f66/58a7f12a-d8d4-447d-88f3-28f51572d6b5.png
...
- **Trigger:** Device A sends a PATCH /shipment/{id} request to the OIC REST endpoint.  
- **Normalization:** The flow extracts the SessionID, DeviceID, and Payload.  
- **Lock Acquisition:** The flow attempts to acquire a distributed lock on the ShipmentID in the Session State Store.  

**Success:** Proceed to validation.  
**Failure:** Return 423 Locked to Device A and queue the request for retry.  

- **Validation:** Compare the incoming VersionNumber (optimistic locking) against the current database record.  
- **Execution:** If valid, update the backend system.  
- **Broadcast:** Publish a "State Changed" event to the message bus (e.g., Kafka or OIC Event Mesh) to notify Device B and C.  
- **Release:** Release the distributed lock.  
